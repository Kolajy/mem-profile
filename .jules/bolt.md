...
## 2024-07-20 - [TUI Memoization of Symbolicated Backtraces]
**Learning:** Calling `symbolicate_frames` inside the TUI's rendering loop (`get_active_allocations`) without caching caused a severe performance bottleneck. The hot loop was redundantly resolving the same raw backtrace pointers (`Vec<*mut std::ffi::c_void>`) into strings multiple times per second, which is a slow, allocation-heavy operation. Adding a `HashMap` cache inside the `App` struct reduced frame generation time from ~285ms to ~9ms.
**Action:** Always memoize the stringification of raw backtraces if they are continuously computed in a hot loop (like a TUI refresh cycle). Additionally, remember that caching `Vec<*mut std::ffi::c_void>` within a struct shared across threads (like `App` via `Arc<Mutex<App>>`) requires manually implementing `unsafe impl Send/Sync` for the struct since raw pointers are not thread-safe by default, but this is sound here as they represent static instruction addresses.
