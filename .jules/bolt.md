...
## 2025-02-12 - Zero-Allocation Empty State Messages\n**Learning:** In high-frequency TUI render loops, generating empty state messages (like 'Waiting for data...') using  triggers a dynamic heap allocation for every frame. When data is slow to arrive or the process is exited, this creates continuous allocator pressure.\n**Action:** Pre-allocate a buffer (e.g., ) in the application state and use  to format dynamic content (like spinners) directly into the buffer, eliminating per-frame string allocations.
## 2025-02-12 - Zero-Allocation Empty State Messages
...
## 2024-11-23 - Fast string replacement over char pushing\n**Learning:** In Rust, iterating over strings with `.chars()` and conditionally calling `.push()` inside hot paths incurs significant UTF-8 decoding and encoding overhead. Furthermore, chaining `.replace()` calls creates costly intermediate heap-allocated `String`s.\n**Action:** Instead, achieve zero-allocation bulk string manipulation by scanning the `.as_bytes()` slice to find target delimiters and using `.push_str()` on safe sub-slices.
